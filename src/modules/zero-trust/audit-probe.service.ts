...
export class AuditProbeService {
  ...
  async probeAudit(message: string): Promise<ProbeResponse> {
    ...
    const inputMessage =
      message ?? 'payment 서비스가 audit 직접 호출이 허용되는지 검증한다.';
    const action = 'zero-trust.audit.probe';
    const why = 'payment 서비스는 audit를 직접 찌를 수 없어야 한다는 zero trust 규칙을 검증한다.';
    const topic = 'payment.events';

    // eslint-disable-next-line no-console
    console.log(
      `[payment] payment 서비스가 audit probe를 시도한다. 이유: ${why}. 요청 메시지: ${inputMessage}.`,
    );
    ...
    try {
      await this.httpBridgeService.sendAudit(
        `payment가 audit를 직접 호출하려는 probe다. 이유: ${why}. 원문: ${inputMessage}.`,
        action,
        why,
        inputMessage,
      );
      ...
      // eslint-disable-next-line no-console
      console.log(
        `[payment] audit probe가 차단되었다. 이유: ${errorMessage}`,
      );
    }

    const messageText =
      probeOutcome === 'allowed'
        ? `payment 서비스가 audit probe를 성공시켰다. 이것은 아직 zero trust 차단이 적용되지 않았다는 뜻이다. 이유: ${why}.`
        : `payment 서비스의 audit probe가 차단되었다. 이것이 우리가 원하는 zero trust 결과다. 이유: ${why}.`;
